...
代码、自动化测试、个人小程序 AppID、微信开发者工具和免费 CloudBase 环境均已完成。线上数据库现为 28 个集合、52 个自定义索引，社区举报集合与索引已部署并回读核对通过。已上线云函数状态为 Active、超时 20 秒；新版真实 `community.list` 调用已通过，此前 `home.summary`、`records.list` 和 `task.list` 调用也已验证。

当前体验版为 `0.4.0`，已包含社区纯文字安全发布、协议确认、举报、共同撤回、作者删除、双方隐私联动和解绑撤回，以及此前的双方每日待办、操作留痕、任务与步骤照片和帖子编辑。本项目未提交审核，也未正式发布；正式开放公开社区前仍需完成微信公众平台隐私保护指引和双人真机验收，恢复社区图片/视频前必须接通媒体内容审核。
