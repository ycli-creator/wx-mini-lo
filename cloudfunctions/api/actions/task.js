...
const create = async ({ openid, payload }) => {
  ...
  const bothRequired = payload.assignee === 'both'
  assert(title, 'INVALID_TITLE', '请填写待办名称')
  assert(title.length <= 60 && description.length <= 200, 'TASK_TOO_LONG', '待办名称或说明超过长度限制')
  assert(Number.isInteger(points) && points > 0 && points <= 10000, 'INVALID_POINTS', '待办积分必须是 1–10000 的整数')
  assert(spaceType !== 'personal' || (taskType === 'personal' && payload.assignee !== 'partner'), 'COUPLE_REQUIRED', '绑定 TA 后才能创建共同待办或指定伴侣完成')
  assert(kind !== 'project' || spaceType === 'couple', 'COUPLE_REQUIRED', '大计划只能创建在情侣空间')
  assert(!bothRequired || (spaceType === 'couple' && kind === 'recurring' && planType === 'daily'), 'BOTH_DAILY_ONLY', '双方完成模式仅用于情侣空间的每日待办')
  const rawSteps = Array.isArray(payload.projectSteps) ? payload.projectSteps : []
  assert(kind !== 'project' || (rawSteps.length >= 2 && rawSteps.length <= 8), 'PROJECT_STEP_COUNT', '大计划需要设置 2–8 个环节')
  ...
  const projectSteps = rawSteps.map((step, index) => {
    ...
    assert(stepTitle && stepTitle.length <= 60, 'PROJECT_STEP_INVALID', '每个大计划环节都需要填写名称，且不能超过 60 个字')
    return {
  ...
  const task = {
    ...
    points,
    assigneeMode: bothRequired ? 'both' : partnerAssigned ? 'partner' : 'self',
    bothRequired,
    kind,
    ...
    reviewerOpenId: bothRequired ? null : partnerAssigned ? openid : partnerId || openid,
  ...
  await writeOperationLog({ coupleId, openid, action: 'task.create', targetId: taskId, detail: bothRequired ? '创建了双方每日都要完成的待办' : '创建了待办' })
...
const submit = async ({ openid, payload }) => {
  ...
  assert(task && cycle, 'TASK_NOT_FOUND', '没有可提交的待办')
  assert(task.kind !== 'project', 'PROJECT_STEP_REQUIRED', '请进入大计划详情完成具体环节')
  ...
  if (task.bothRequired || task.assigneeMode === 'both') {
    assert(spaceType === 'couple' && planType === 'daily', 'BOTH_DAILY_ONLY', '双方完成模式仅用于情侣空间的每日待办')
    let finalStatus = cycle.status
    let newlyCompleted = false
    await db.runTransaction(async (transaction) => {
      const latestCycle = (await transaction.collection(collections.taskCycles).doc(cycle._id).get()).data
      assert(latestCycle && ['todo', 'partial', 'approved'].includes(latestCycle.status), 'TASK_STATE_CHANGED', '待办状态已变化，请刷新后重试')
      const completions = { ...(latestCycle.participantCompletions || {}) }
      if (completions[openid]) { finalStatus = latestCycle.status; return }
      newlyCompleted = true
      completions[openid] = { completedAt: now(), note, evidence }
      const bothDone = Boolean(completions[openid] && partnerId && completions[partnerId])
      finalStatus = bothDone ? 'approved' : 'partial'
      ...
      const balanceAfter = Number(account.sharedBalance || 0) + Number(task.points || 0)
      const ledgerId = `ledger_task_participant_${cycle._id}_${openid}`
      ...
      await transaction.collection(collections.ledgers).doc(ledgerId).set({ data: {
        coupleId, accountOwnerOpenId: openid, pointsType: 'shared', direction: 'credit', amount: Number(task.points || 0), balanceAfter,
        sourceType: 'task_participant_completion', sourceId: cycle._id, actorOpenId: openid, title: task.title,
        detail: `${cycle.cycleLabel || cycle.cycleKey}双方待办个人完成`, idempotencyKey: ledgerId, createdAt: now(),
      } })
      await transaction.collection(collections.taskCycles).doc(cycle._id).update({ data: { status: finalStatus, participantCompletions: completions, latestNote: note, evidence, rejectionReason: '', settledAt: bothDone ? now() : null, updatedAt: now() } })
    })
    if (newlyCompleted) {
      await writeOperationLog({ coupleId, openid, action: 'task.participant.complete', targetId: cycle._id, detail: `完成了今日待办，情侣积分 +${task.points}` })
      if (partnerId) await createNotification({ recipientOpenId: partnerId, coupleId, type: 'task', title: finalStatus === 'approved' ? '双方今日都已完成' : 'TA 已完成今日待办', body: `“${task.title}”的双人进度已更新`, actionPath: `/pages/task/detail?id=${cycle._id}`, sourceId: cycle._id })
      await heat.grant({ openid, code: 'HF03', participantOpenId: openid, businessResourceId: `${cycle._id}:${openid}` })
      if (finalStatus === 'approved') await heat.grant({ openid, code: 'HR01', businessResourceId: cycle._id })
    }
    return projectState(openid)
  }

  assert(task.assigneeOpenId === openid, 'FORBIDDEN', '只有待办执行人可以提交')

  const submissionId = makeId('submission')
  await db.runTransaction(async (transaction) => {
    const latestCycle = (await transaction.collection(collections.taskCycles).doc(cycle._id).get()).data
    assert(latestCycle && ['todo', 'rejected'].includes(latestCycle.status), 'TASK_STATE_CHANGED', '待办状态已变化，请刷新后重试')
  ...
  await writeOperationLog({ coupleId, openid, action: 'task.submit', targetId: cycle._id, detail: '提交了完成记录，等待对方确认' })
  if (task.reviewerOpenId !== openid) await createNotification({ recipientOpenId: task.reviewerOpenId, coupleId, type: 'task', title: '待办待确认', body: `“${task.title}”已经提交完成`, actionPath: '/pages/task/index', sourceId: cycle._id })
...
const update = async ({ openid, payload }) => {
  ...
  const { task } = await resolveTaskCycle(coupleId, String(payload.taskId || ''))
  assert(task, 'TASK_NOT_FOUND', '待办不存在或已经删除')
  const title = String(payload.title || '').trim()
  const description = String(payload.description || '').trim()
  const points = Number(payload.points)
  const completionRequirement = ['direct', 'note', 'image'].includes(payload.completionRequirement) ? payload.completionRequirement : task.completionRequirement
  assert(title, 'INVALID_TITLE', '请填写待办名称')
  assert(title.length <= 60 && description.length <= 200, 'TASK_TOO_LONG', '待办名称或说明超过长度限制')
  assert(Number.isInteger(points) && points > 0 && points <= 10000, 'INVALID_POINTS', '待办积分必须是 1–10000 的整数')
  if (task.kind === 'project' && (task.projectSteps || []).some((step) => step.status === 'done')) assert(points === Number(task.points), 'PROJECT_POINTS_LOCKED', '大计划已有完成环节，不能再修改积分')
  const changed = []
  if (task.title !== title) changed.push('名称')
  if (String(task.description || '') !== description) changed.push('说明')
  if (Number(task.points || 0) !== points) changed.push('积分')
  if (task.kind !== 'project' && task.completionRequirement !== completionRequirement) changed.push('完成方式')
  const data = { title, description, points, updatedAt: now() }
  if (task.kind !== 'project') data.completionRequirement = completionRequirement
  if (task.kind === 'project' && Number(task.points || 0) !== points) data.projectSteps = (task.projectSteps || []).map((step) => ({ ...step, rewardPoints: Math.floor(points * 0.1) }))
  await db.collection(collections.tasks).doc(task._id).update({ data })
  await writeOperationLog({ coupleId, openid, action: 'task.update', targetId: task._id, detail: changed.length ? `修改了${changed.join('、')}` : '保存了待办信息' })
  return projectState(openid)
}

const addMedia = async ({ openid, payload }) => {
  const { coupleId } = await requireSpace(openid)
  const { task } = await resolveTaskCycle(coupleId, String(payload.taskId || ''))
  assert(task, 'TASK_NOT_FOUND', '待办不存在或已经删除')
  const media = (Array.isArray(payload.media) ? payload.media : []).filter((item) => item?.type === 'image' && item.fileId).slice(0, 9)
  assert(media.length, 'TASK_MEDIA_REQUIRED', '请选择照片')
  const stepId = String(payload.stepId || '')
  let detail = `为待办添加了 ${media.length} 张照片`
  await db.runTransaction(async (transaction) => {
    const latestTask = (await transaction.collection(collections.tasks).doc(task._id).get()).data
    assert(latestTask, 'TASK_NOT_FOUND', '待办不存在或已经删除')
    if (stepId) {
      const steps = Array.isArray(latestTask.projectSteps) ? latestTask.projectSteps : []
      const step = steps.find((item) => item.id === stepId)
      assert(step, 'PROJECT_STEP_NOT_FOUND', '计划环节不存在')
      detail = `为环节“${step.title}”添加了 ${media.length} 张照片`
      const updatedSteps = steps.map((item) => item.id === stepId ? { ...item, media: [...(Array.isArray(item.media) ? item.media : []), ...media].slice(0, 9) } : item)
      await transaction.collection(collections.tasks).doc(task._id).update({ data: { projectSteps: updatedSteps, updatedAt: now() } })
    } else {
      await transaction.collection(collections.tasks).doc(task._id).update({ data: { media: [...(Array.isArray(latestTask.media) ? latestTask.media : []), ...media].slice(0, 9), updatedAt: now() } })
    }
  })
  await writeOperationLog({ coupleId, openid, action: 'task.media.add', targetId: stepId || task._id, detail })
  return projectState(openid)
}

const completeProjectStep = async ({ openid, payload }) => {
  const { coupleId, partnerId, spaceType } = await requireSpace(openid)
  assert(spaceType === 'couple', 'COUPLE_REQUIRED', '大计划只能在情侣空间完成')
  const referenceId = String(payload.taskId || '')
  const { task } = await resolveTaskCycle(coupleId, referenceId)
  assert(task && task.kind === 'project', 'PROJECT_NOT_FOUND', '大计划不存在')
  const stepId = String(payload.stepId || '')
  ...
  await db.runTransaction(async (transaction) => {
    ...
    await transaction.collection(collections.ledgers).doc(ledgerId).set({ data: {
      ...
      detail: '完成大计划环节', idempotencyKey: ledgerId, createdAt: now(),
    } })
    await transaction.collection(collections.tasks).doc(task._id).update({ data: { projectSteps: updatedSteps, updatedAt: now() } })
  })
  await writeOperationLog({ coupleId, openid, action: 'task.project.step.complete', targetId: step.id, detail: `完成环节“${step.title}”，情侣积分 +${rewardPoints}` })
...
const completeProject = async ({ openid, payload }) => {
  ...
  assert(task && cycle && task.kind === 'project', 'PROJECT_NOT_FOUND', '大计划不存在')
  assert((task.projectSteps || []).length >= 2 && task.projectSteps.every((step) => step.status === 'done'), 'PROJECT_NOT_READY', '完成所有环节后才能结束大计划')
  ...
  await db.runTransaction(async (transaction) => {
    ...
    assert((latestTask.projectSteps || []).every((step) => step.status === 'done'), 'PROJECT_NOT_READY', '完成所有环节后才能结束大计划')
    const account = (await transaction.collection(collections.accounts).doc(coupleId).get()).data
  ...
  await writeOperationLog({ coupleId, openid, action: 'task.project.complete', targetId: task._id, detail: '完成了整个大计划并结算剩余积分' })
...
const review = async ({ openid, payload }) => {
  ...
  assert(task.reviewerOpenId === openid, 'FORBIDDEN', '只有指定确认人可以处理待办')
  ...
  await db.runTransaction(async (transaction) => {
    ...
    await transaction.collection(collections.ledgers).doc(ledgerId).set({ data: {
      ...
      actorOpenId: openid,
      title: task.title,
      detail: `${cycle.cycleLabel || cycle.cycleKey}待办确认完成`,
  ...
  await writeOperationLog({ coupleId, openid, action: payload.approved ? 'task.review.approve' : 'task.review.reject', targetId: cycle._id, detail: payload.approved ? `确认完成，${task.pointsType === 'shared' ? '情侣' : '个人'}积分 +${task.points}` : `要求补充完成记录${String(payload.reason || '').trim() ? `：${String(payload.reason || '').trim()}` : ''}` })
  if (task.assigneeOpenId !== openid) await createNotification({ recipientOpenId: task.assigneeOpenId, coupleId, type: 'task', title: payload.approved ? '待办已确认' : '待办需补充', body: `“${task.title}”的确认结果已更新`, actionPath: '/pages/task/index', sourceId: cycle._id })
...
module.exports = { list, create, update, addMedia, submit, review, completeProjectStep, completeProject }
