...
const normalizeTask = (task: TaskItem, at: Date): TaskItem => {
  ...
  const emptyCompletion = { completed: false, completedAt: '', note: '', evidence: [] }
  ...
  return {
    ...
    projectSteps: projectSteps.map((step) => ({ ...step, media: Array.isArray(step.media) ? step.media : [] })),
    ...
    progressPercent: kind === 'project'
      ...
      : bothRequired ? (selfCompletion.completed ? 50 : 0) + (partnerCompletion.completed ? 50 : 0)
